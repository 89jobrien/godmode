...
File: `.ctx/review/02-solid-modules.md`
...
File: `.ctx/review/03-test-coverage.md`
...
After all 5 agents complete, read all 5 `.ctx/review/` files.
Produce `.ctx/review/00-synthesis.md` with a prioritized table:
...
For each HIGH priority finding, dispatch one godmode:tdd-crate-agent (cap 5 concurrent).
Each agent:
...
After all fix agents complete, merge sequentially and run full suite.
Report: findings resolved, findings deferred (MED/LOW), total tests passing.
