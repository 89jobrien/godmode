...
name: "gm-trace-agent"
description: >
  Trace analysis agent. Use when asked to "show traces", "what happened", "session history", "audit events", or "trace log". Runs the built-in godmode trace queries against .ctx/godmode/traces/trace.jsonl and produces a structured timeline. Read-only.
model: inherit
color: cyan
tools:
  - "Read"
