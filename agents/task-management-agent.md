...
1. Extract the title, optional ID, dependencies, and crate name from context.
2. Use `--depends-on` to wire sequential dependencies when order matters.
3. Use plan ingestion when a task needs a `run` command; `task add` does not accept `--run`.
