...
use anyhow::Result;
use godmode_core::{
    agent as core_agent, builder, detect, insights, memory_banking, pipeline, plan, policy,
    registry, release, review, session::Session, skill, workflow,
};
...
pub(crate) fn dispatch(root: std::path::PathBuf, json: bool, sarif: bool, cmd: Cmd) -> Result<()> {
    match cmd {
        ...
        Cmd::Trace { action } => run_trace_action(&root, json, action),
        ...
        Cmd::Agent { action } => run_agent_action(root, json, action),
        ...
        Cmd::Workflow { action } => match action {
            WorkflowAction::Run {
                ...
                let agent_def = core_agent::load(&agent_path)?;
            ...
            WorkflowAction::List {
                ...
                if agents_dir.exists() {
                    ...
                    for yf in yaml_files {
                        let Ok(a) = core_agent::load(&yf) else {
                            continue;
                        };
