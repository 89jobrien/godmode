...
metadata:
  version: "0.23.0"
  author: "Inkeep"
  repository: "github:inkeep/open-knowledge"
---
...
- **Degrees of freedom** (calibrate body precision to task fragility):
  _high_ (free prose — judgment tasks), _medium_ (parameterized steps), _low_
...
- **Body** = the durable, reusable instructions — under ~500 lines. If it's
  growing past that, move depth into `references/description-optimization.md` (loaded only when
  needed) and point at it from the body. For a **project** skill the reference
  auto-connects in the graph either way, so a backticked `references/description-optimization.md`
  path is fine; use a `[[references/description-optimization]]` wiki-link only when you want the
...
Add depth files — Markdown files under `references/` (loaded on demand) and `scripts/*` (shown as
...
# `references/tiers.md` is an example path, not a required built-in file.
...
a backticked `references/description-optimization.md` path joins the graph just like a
`[[references/description-optimization]]` wiki-link. Reach for a wiki-link (or
`[description guidance](references/description-optimization.md)`) only when you want a clickable inline link.
