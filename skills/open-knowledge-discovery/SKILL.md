...
including single files that are not part of a project (see _Opening a file
outside a project_ below). It does **not** carry the in-project read/write
...
project-local skill installed by `ok init` (see _Working inside a project_
below).
...
- **You have an in-app / built-in browser** (Cursor, Codex, and similar) — this
  ...
  port.
- **You have a Claude Code Desktop preview _pane_ but no general browser** — the
