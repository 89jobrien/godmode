...
def main(argv=None):
    ...
    if remaining:
        cycle_names = ", ".join(sorted(nodes[node] for node in remaining))
        raise RuntimeError(f"cycle detected in dependency graph: {cycle_names}")
    ...
    for ring_idx, crate_names in rings_by_idx.items():
        ...
        for name in crate_names:
            ...
            svg_parts.append(
                ...
                f'font-family="Helvetica" font-size="12" font-weight="500" fill="{ring_colors[ci]}">{escape(name)}</text>'
            )
    ...
    for src, dst, _ in reduced:
        ...
        if gap == 1:
            ...
            if gap == 0:
                ...
                severity = "low"
                remedy = (
                    f"Both are in {src_rname}. Consider whether <code>{escape(dst_name)}</code> "
                ...
                remedy = (
                    ...
                    f"<code>{escape(src_name)}</code> only reaches one ring inward."
                )
            else:
                ...
                remedy = (
                    ...
                    f"Extract the interface <code>{escape(src_name)}</code> needs from "
                    f"<code>{escape(dst_name)}</code> into a trait in the inner ring, "
        ...
        for i, a in enumerate(action_items, 1):
          ...
          <div class="action-item" style="border-left:3px solid {"#f44336" if a["severity"] in ("critical", "high") else "#ffa726" if a["severity"] == "medium" else "#448aff"}">
            <div class="action-head">
              ...
              <span class="action-path">{escape(a["src"])} &rarr; {escape(a["dst"])}</span>
              <span style="font-size:11px;color:#484f58">{escape(a["src_ring"])} &rarr; {escape(a["dst_ring"])}</span>
            </div>
    ...
    for date in sorted(by_date.keys()):
        ...
        highlights = "".join(
            f'<div class="tl-msg">{escape(c["msg"][:80])}</div>' for c in commits[:2]
        )
        if len(commits) > 2:
        ...
        timeline_rows += f"""
          <div class="tl-row">
            <div class="tl-date">{escape(date)}</div>
    ...
    for i, rname in enumerate(ring_names):
        ...
        cl = " ".join(f"<code>{escape(c)}</code>" for c in crates)
    ...
    if nodes:
        highest_in = nodes[max(nodes, key=lambda node: in_deg[node])]
        highest_in_count = max(in_deg.values(), default=0)
        highest_out = nodes[max(nodes, key=lambda node: out_deg[node])]
        highest_out_count = max(out_deg.values(), default=0)
    else:
        highest_in = highest_out = "none"
        highest_in_count = highest_out_count = 0
    table_rows = ""
    ...
    <title>{escape(os.path.basename(REPO))} Architecture Report</title>
    <style>
    *{{margin:0;padding:0;box-sizing:border-box}}
    ...
    <header>
      <h1>{escape(os.path.basename(REPO))} Workspace Architecture</h1>
      <p><code>{escape(branch)}</code> &middot; diverged from <code>{escape(base)}</code> at
        <code>{escape(mb[:11])}</code> ({escape(mb_date)}) &middot;
    ...
    <section>
      <h2>Evolution Timeline (divergence from {escape(base)})</h2>
    ...
    <section>
      ...
      <div class="insights">
        <ul>
          ...
          <li><strong>Highest fan-in:</strong> {escape(highest_in)}
            ({highest_in_count} direct dependents) &mdash; widest recompilation blast radius.</li>
          <li><strong>Highest fan-out:</strong> {escape(highest_out)}
            ({highest_out_count} direct deps) &mdash; main integration point.</li>
          ...
          <li><span style="color:#4caf50"><strong>{good_edges} good deps</strong></span> (adjacent ring),
            ...
            {(" Bad: " + escape("; ".join(bad_edge_list)) + ".") if bad_edge_list else ""}</li>
        </ul>
      </div>
    </section>
